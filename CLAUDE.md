...
- Before editing, run `git fetch origin main` and create a dedicated task branch and worktree from the freshly fetched `origin/main`. Never branch from an unrelated task branch or an unchecked local `main`.
...
- Use the shared `kaneo-task` skill at `~/Documents/github/personal/.agents/skills/kaneo-task/SKILL.md` to create or strengthen tasks and ADRs. Check for an existing task before creating another.
