...
import { GraphSpatial, type GraphMode } from "./graph-spatial"
...
function GraphCanvas({ data, currentSlug, selected, onSelect, expanded = false }: {
  ...
  useEffect(() => {
    ...
    async function draw() {
      ...
      const motionDuration = () => window.matchMedia("(prefers-reduced-motion: reduce)").matches ? 0 : 240
...
export function GraphView({ currentSlug }: { currentSlug?: string }) {
  ...
  useEffect(() => { setSelected(currentSlug); setShowAll(false); setMode("2d") }, [currentSlug])
    ...
    <details aria-label="관련 콘텐츠 그래프" open={open} onToggle={event => setOpen(event.currentTarget.open)} className="mt-6 border-t pt-4">
      ...
      {open && (
        <>
          ...
          <Dialog open={expanded} onOpenChange={value => { setExpanded(value); if (!value) setMode("2d") }}>
            <DialogContent onCloseAutoFocus={event => { event.preventDefault(); exploreRef.current?.focus() }} className="graph-dialog max-h-[92dvh] gap-0 overflow-y-auto rounded-2xl p-0 sm:max-w-6xl">
              ...
              <div className="flex flex-wrap items-center gap-3 border-b px-4 py-3 sm:px-6">
                <div role="group" aria-label="그래프 보기 방식" className="flex rounded-lg bg-muted p-1 text-xs font-medium">
                  {(["2d", "3d", "vr", "ar"] as const).map(view => <button key={view} aria-pressed={mode === view} onClick={() => setMode(view)} className={`rounded-md px-3 py-1.5 text-foreground transition-colors ${mode === view ? "bg-background shadow-sm" : "hover:bg-background/60"}`}>{view.toUpperCase()}</button>)}
                </div>
              ...
              <div className="grid min-h-0 sm:grid-cols-[minmax(0,1fr)_17rem]">
                {graph && expanded && (mode === "2d" ? <GraphCanvas data={graph} currentSlug={currentSlug} selected={selectedNode?.id} onSelect={setSelected} expanded /> : <GraphSpatial key={mode} mode={mode} data={graph} selected={selectedNode?.id} onSelect={setSelected} onExit={() => setMode("2d")} />)}
                <aside aria-label="Selected note" className="flex min-h-0 flex-col border-t bg-background sm:max-h-[58dvh] sm:min-h-96 sm:border-l sm:border-t-0">
                  ...
                  {query.trim() ? <div className="max-h-64 overflow-y-auto p-2 sm:max-h-none" aria-label="Graph search results">
                    ...
                    <div key={selectedNode.id} className="graph-selection p-5">
                      ...
                      <Link href={graphNodeHref(selectedNode)} onClick={() => { setExpanded(false); setMode("2d") }} className="mt-4 inline-flex items-center gap-1.5 rounded-lg bg-teal-700 px-3 py-2 text-xs font-medium text-white hover:bg-teal-800 dark:bg-teal-300 dark:text-teal-950 dark:hover:bg-teal-200">{selectedNode.type === "tag" ? "Browse tag" : "Open note"}<ArrowUpRight className="size-3.5" /></Link>
                    </div>
              ...
              <div className="flex flex-wrap justify-between gap-2 border-t px-5 py-3 text-[11px] text-muted-foreground"><span>{mode === "2d" ? "드래그로 배치 · 스크롤로 확대" : mode === "3d" ? "드래그로 회전 · 스크롤로 확대 · 클릭으로 선택" : mode === "vr" ? "헤드셋 연결은 지원 기기와 브라우저에서 가능합니다." : "카메라와 Hiro 마커로 그래프를 표시합니다."}</span><span>{mode === "2d" ? "방향키로 이동 · Enter로 선택" : "검색과 연결된 글 탐색을 함께 사용할 수 있습니다."}</span></div>
            </DialogContent>
          </Dialog>
        </>
      )}
