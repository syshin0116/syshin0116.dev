...
export function TableOfContents({ toc, className, children, mobile = false }: {
  ...
  const nav = toc.length > 0 && <nav aria-label="목차" onClick={event => {
  ...
  if (mobile) return <details ref={detailsRef} className="reading-toc mb-6 rounded-lg border bg-muted/20 xl:hidden">
    <summary className="flex cursor-pointer list-none items-center gap-2 px-3 py-3 text-sm font-medium"><span className="shrink-0">목차</span><span className="ml-auto truncate text-xs font-normal text-muted-foreground">{findActive(toc)}</span><ChevronDown className="size-4 shrink-0" /></summary>
  ...
  return <aside className={cn("hidden w-[var(--toc-width)] shrink-0 xl:block", className)}>
    <div className="sticky top-14 max-h-[calc(100dvh-3.5rem)] overflow-y-auto py-4 pl-3">
      {toc.length > 0 && <><p className="mb-3 text-xs font-semibold text-muted-foreground">목차</p>{nav}<a href="#main-content" className="mt-4 flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground"><ArrowUp className="size-3" />맨 위로</a></>}
      {children}
    </div>
  </aside>
}
...
function TocList({ entries, activeId, nested = false }: { entries: TocEntry[]; activeId: string; nested?: boolean }) {
  return <ul className={cn("space-y-0.5", nested && "ml-3")}>
    {entries.map(entry => <li key={entry.id}>
      <a href={`#${entry.id}`} aria-current={activeId === entry.id ? "location" : undefined} className={cn("block rounded-md px-2 py-1.5 text-[13px] leading-snug hover:text-foreground", activeId === entry.id ? "bg-muted font-medium text-foreground" : "text-muted-foreground")}>{entry.text}</a>
